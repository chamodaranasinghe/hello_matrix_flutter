...
import 'package:hello_matrix_flutter_example/main.dart';
...
class Signaling {
      ...
      case 'offer':
        {
          ...
          var sessionId = data['session_id'];
          var session = _sessions[sessionId];
          onCallStateChange?.call(null, CallState.CallStateRinging);
          var newSession = await _createSession(
              session: session,
              peerId: peerId,
              sessionId: sessionId,
              ...
              screenSharing: false);
          _sessions[sessionId] = newSession;
          //set variables for create answer
          ...
          await CallKeep.displayIncomingCall(
              sessionId, peerId, peerId, HandleType.generic, false);
        }
        break;
  ...
  accept() async {
    print('accepted');
    await _newSession.pc.setRemoteDescription(
        RTCSessionDescription(_description['sdp'], _description['type']));
    await _createAnswer(_newSession, _media);
    if (_newSession.remoteCandidates.length > 0) {
      _newSession.remoteCandidates.forEach((candidate) async {
        await _newSession.pc.addCandidate(candidate);
      });
      _newSession.remoteCandidates.clear();
    }
    onCallStateChange?.call(_newSession, CallState.CallStateNew);
  }

  reject() {
    print('rejected');
    bye(_newSession.sid);
    _closeSessionByPeerId(_newSession.pid);
  }
    ...
    if (_turnCredential == null) {
      try {
        ...
        */
        ...
        print('ice servers ${_turnCredential['uris'].runtimeType}');
      } catch (e) {}
    }
  ...
  Future<Session> _createSession(
    ...
    pc.onSignalingState = (RTCSignalingState signalingState) {
      print('RTCSignalingState $signalingState');
    };
    pc.onConnectionState = (RTCPeerConnectionState rtcPeerConnectionState) {
      print('RTCPeerConnectionState $rtcPeerConnectionState');
    };
...
*/
  Future<void> _createOffer(Session session, String media) async {
    try {
      ...
      await session.pc.setLocalDescription(rtcSessionDescription);
      _send('offer', {
        'to': session.pid,
        'from': _selfId,
        'description': {
...
final signallingInstance = getIt.get<Signaling>();
